...
$("#Q2_submit").click(function(){
	...
	if($("input:checkbox[name=words]:checked").length>1){
		$("p.Q2.more").show();
	}
	else if($("input:checkbox[name=words]:checked").length<1){
		$("p.Q2.less").show();
	}else{
		submit_time++;
		var correct = [];
		...
		$("input:checkbox[name=words]:checked").each(function(){
			if($(this).hasClass("1-4"))
				correct.push($(this).val());
			else
				incorrect.push($(this).val());
		});
		if(submit_time == 1 && correct.length == 1){
			score++;
			$("#score_span").text(score);
			$("#this_span").text("1");
			$("p.Q2.all").show();
